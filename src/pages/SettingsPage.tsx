...
export default function SettingsPage({ onBack }: SettingsPageProps) {
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = `kacha-backup-${new Date().toISOString().slice(0, 10)}.json`
  ...
  const handleImport = () => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        try { importAllData(JSON.parse(ev.target?.result as string)); alert('导入成功！') }
      ...
      <div className="page">
        ...
        <div className="card">
          <div className="flex-between">
            <div><div className="card-title">外观</div><div className="card-subtitle">{settings.theme === 'dark' ? '深色模式' : '浅色模式'}</div></div>
        ...
        <div className="card">
          <div className="flex-between">
            <div><div className="card-title">语言</div><div className="card-subtitle">{getLanguage() === 'zh' ? '中文' : 'English'}</div></div>
            <div style={{ display: 'flex', gap: 6 }}>
              <span className={`tag ${getLanguage() === 'zh' ? 'tag-primary' : ''}`} style={{ cursor: 'pointer' }}
                onClick={() => setLanguage('zh')}>中文</span>
              <span className={`tag ${getLanguage() === 'en' ? 'tag-primary' : ''}`} style={{ cursor: 'pointer' }}
          ...
          {settings.dndEnabled && (
            <div style={{ display: 'flex', gap: 12 }}>
              <div className="form-group" style={{ flex: 1 }}>
                ...
                <input className="form-input" type="time" value={settings.dndStart} onChange={(e) => updateSettings({ dndStart: e.target.value })} />
              </div>
              <div className="form-group" style={{ flex: 1 }}>
                <label className="form-label">结束</label>
                <input className="form-input" type="time" value={settings.dndEnd} onChange={(e) => updateSettings({ dndEnd: e.target.value })} />
              </div>
            </div>
          )}
        </div>
        {/* Floating window */}
        <FloatingWindow
          mode={settings.floatingWindowMode}
          enabled={settings.floatingWindowEnabled}
          onToggle={(v) => updateSettings({ floatingWindowEnabled: v })}
          onModeChange={(m) => updateSettings({ floatingWindowMode: m })}
        />
        {/* OCR Server */}
        <div className="card">
          <div className="card-title mb-sm">云端OCR</div>
          <div className="form-group">
            <label className="form-label">服务器地址</label>
            <input className="form-input" value={ocrUrl} onChange={(e) => setOcrUrl(e.target.value)} placeholder="http://your-server:8080" />
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-primary btn-sm" onClick={() => updateSettings({ ocrServerUrl: ocrUrl })}>保存</button>
            <button className="btn btn-ghost btn-sm" onClick={() => { if (!ocrUrl) { setTestResult('请输入地址'); return }; setTestResult('测试中...'); setTimeout(() => setTestResult('连接成功'), 2000) }}>测试连接</button>
        ...
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 12, color: 'var(--color-text-tertiary)' }}>咔嚓 App v3.5</div>
        </div>
      </div>
    </>
  )
