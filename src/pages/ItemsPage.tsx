...
interface ItemsPageProps { onNavigate: (page: PageName) => void }

export default function ItemsPage({ onNavigate }: ItemsPageProps) {
  const items = [
    { key: 'expiry' as PageName, label: '物品管理', icon: '📦', desc: '保质期/保修期' },
    { key: 'coupon' as PageName, label: '优惠券', icon: '🎫', desc: '券码管理' },
    { key: 'expense' as PageName, label: '报销', icon: '💰', desc: '费用记录' },
  ]
  return (
    <>
      <div className="page-header"><h1>📦 物品</h1></div>
      <div className="page">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 8 }}>
          {items.map((item) => (
            <div key={item.key} className="card" style={{ cursor: 'pointer', textAlign: 'center', padding: '24px 12px' }} onClick={() => onNavigate(item.key)}>
